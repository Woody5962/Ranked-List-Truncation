...
class MtChoopy(nn.Module):
    def __init__(self, d_model: int=128, n_head: int=8, num_layers: int=3, num_tasks: float=3, dropout: float=0.4):
        super(MtChoopy, self).__init__()
        self.num_tasks = num_tasks
        self.position_encoding = nn.Parameter(t.randn(300, 127), requires_grad=True)
    ...
    def forward(self, x):
        pe = self.position_encoding.expand(x.shape[0], 300, 127)
        x = t.cat((x, pe), dim=2)
